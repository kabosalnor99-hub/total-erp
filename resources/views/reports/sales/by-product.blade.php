...
<div class="p-6" dir="rtl">
    ...
    <div class="mb-6 flex items-center justify-between">
        <div>
            <h1 class="text-2xl font-bold text-gray-800">المبيعات حسب المنتج</h1>
            <p class="text-sm text-gray-500 mt-1">تحليل الكميات والمبالغ المباعة لكل منتج</p>
        </div>
    ...
    <form method="GET" class="bg-white rounded-xl border border-gray-200 p-4 mb-6">
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div class="flex items-end gap-2">
                ...
                <a href="{{ route('reports.sales-by-product') }}" class="border border-gray-300 text-gray-600 px-4 py-2 rounded-lg text-sm hover:bg-gray-50 transition">إعادة تعيين</a>
            </div>
        </div>
    </form>
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
        ...
        <div class="bg-white rounded-xl border border-gray-200 p-4">
            <p class="text-xs text-gray-500 mb-1">عدد المنتجات المباعة</p>
            <p class="text-2xl font-bold text-gray-800">{{ $data['rows']->count() }}</p>
            <p class="text-xs text-gray-400 mt-1">منتج</p>
        </div>
        <div class="bg-white rounded-xl border border-green-200 p-4">
            <p class="text-xs text-gray-500 mb-1">إجمالي الوحدات المباعة</p>
            <p class="text-2xl font-bold text-green-700">{{ number_format($data['rows']->sum('total_qty'), 0) }}</p>
            <p class="text-xs text-gray-400 mt-1">وحدة</p>
        </div>
    </div>
    ...
    <div class="bg-white rounded-xl border border-gray-200 overflow-hidden">
        <div class="px-5 py-4 border-b border-gray-100">
            <h2 class="font-semibold text-gray-700">تفصيل المبيعات حسب المنتج</h2>
        </div>
            ...
            <table class="w-full text-sm">
                <thead class="bg-gray-50 border-b border-gray-200">
                    <tr>
                        ...
                        <th class="px-4 py-3 text-right font-semibold text-gray-600">الرمز</th>
                        <th class="px-4 py-3 text-center font-semibold text-gray-600">الكمية المباعة</th>
                        <th class="px-4 py-3 text-left font-semibold text-gray-600">إجمالي الإيرادات</th>
                    ...
                    <tr class="hover:bg-gray-50">
                        ...
                        <td class="px-4 py-3">
                            <span class="font-medium text-gray-800">
                                {{ $row->product->name_ar ?? $row->product->name_en ?? 'منتج محذوف' }}
                            </span>
                        </td>
                        <td class="px-4 py-3 text-gray-500 text-xs font-mono">{{ $row->product->sku ?? '-' }}</td>
                        <td class="px-4 py-3 text-center text-gray-700">{{ number_format($row->total_qty, 2) }}</td>
                ...
                <tfoot class="bg-gray-50 border-t border-gray-200">
                    <tr>
                        <td colspan="3" class="px-4 py-3 font-semibold text-gray-700">الإجمالي</td>
                        <td class="px-4 py-3 text-center font-bold text-gray-700">{{ number_format($data['rows']->sum('total_qty'), 2) }}</td>
